...
redisClientSub
  ...
  .subscribe(redis.CHANNEL)(function (error) {
    if (error) {
...
exports.addUserConsumer = function *(userId, consumerId, simple) {
  var userKey = redis.genUserStateKey(userId)
  debug('addUserConsumer:', userId, consumerId)
  if (simple) yield redisClient.sadd(userKey, consumerId)
  else {
    yield [
...
function pullMessage (consumerId) {
  ...
  var queueKey = redis.genQueueKey(consumerId)
