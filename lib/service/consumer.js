...
const redisClientSub = redis.getClient()
...
// generate consumerId from session
exports.genConsumerId = function * (session) {
  let id = tool.session2ID(session)
  let isMember = yield redisClient.sismember(redis.genUserStateKey(session.userId), id)
  if (!isMember) return id
  session.exp++
  return yield exports.genConsumerId(session)
}
...
function pullMessage (consumerId) {
  ...
  redisClient.lrange(queueKey, 1, redis.DEFT_NUM_MESSAGES_TO_PULL)(function * (err, messages) {
    if (err) throw err
    ...
    yield redisClient.ltrim(queueKey, messages.length, -1)
