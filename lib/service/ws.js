  ...
  const wsServer = new engine.Server({
    ...
    allowRequest: (req, callback) => {
      thunk(function * () {
        ...
        req.session.id = tool.session2ID(req.session)
        ...
        if (err == null) callback(null, true)
          ...
          if (!app.isTokenError(err)) {
            ilog.error(Object.assign(err, {class: 'ws-verify', query: req._query}))
          }
          callback(3, false) // eslint-disable-line
        }
      })
    }
  })
...
function onMessage (data) {
  ...
  if (!this.pendingRPC || res.payload.id !== this.pendingRPC.id) {
    ...
    ilog.warning(Object.assign(new Error('invalid message'), {
      class: 'ws-message',
      id: this.id,
      data: res
    }))
