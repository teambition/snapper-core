...
const server = module.exports = net.createServer(function (socket) {
...
redisClient.clientReady(() => server.start())
...
function onData (message) {
  ...
  if (req.type !== 'request') {
    ...
    if (socket.invalidRequestCount > DEFT_MAX_INVALID_REQ_NUM) {
      socket.end(Resp.encodeError(new Error('excessive invalid requests')))
    }
    return
  }
